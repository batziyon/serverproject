...
function ListPage({
  ...
  fetchData, // הפונקציה שמביאה את המידע מהשרת (מועברת מהאבא כי כל דף טוען משהו אחר)
  ...
  // --- פונקציית הטעינה ---
  const loadData = async (pageNum) => { // פונקציה אסינכרונית לטעינת מידע, מקבלת מספר עמוד
    setIsLoading(true); // מסמנים שהתחלנו לטעון (מפעיל ספינר או הודעה)
    try {
      const data = await fetchData(pageNum, limit); // קריאה לפונקציה שהאבא שלח, כדי להביא מידע מהשרת
      if (data.length < limit) setHasMore(false); // אם הגיעו פחות מ-10 פריטים, סימן שזה הסוף ואין עוד מה לטעון

      if (pageNum === 1) { // אם זה העמוד הראשון (טעינה ראשונית או רענון)
        setItems(data); // מכניסים את המידע ל-state הראשי
        setFiltered(data); // מכניסים גם ל-state המוצג
      } else { // אם זו טעינת המשך ("טען עוד")
        setItems(prev => { // מעדכנים את ה-state על סמך מה שהיה קודם
          const existingIds = new Set(prev.map(item => item.id)); // יוצרים סט של כל ה-ID שכבר יש לנו (למניעת כפילויות)
          const uniqueNewItems = data.filter(item => !existingIds.has(item.id)); // מסננים מהחדשים רק את מה שלא קיים אצלנו
          return [...prev, ...uniqueNewItems]; // מחזירים מערך חדש: הישנים + החדשים הייחודיים
        });
        setFiltered(prev => { // עושים אותו דבר בדיוק לרשימה המסוננת
          const existingIds = new Set(prev.map(item => item.id)); // מזהים קיימים
          const uniqueNewItems = data.filter(item => !existingIds.has(item.id)); // סינון כפילויות
          return [...prev, ...uniqueNewItems]; // הוספה לסוף
        });
      }
    } catch (error) { // אם הייתה שגיאה בטעינה
      console.error("Error loading data:", error); // מדפיסים לקונסול
    } finally { // תמיד קורה בסוף (בין אם הצליח ובין אם נכשל)
      setIsLoading(false); // מפסיקים את סימון הטעינה
    }
  };

  useEffect(() => { // אפקט שרץ כשהקומפוננטה עולה או כשפונקציית הטעינה משתנה
    setPage(1); // מאפסים לעמוד 1
    setHasMore(true); // מאפסים שיש עוד מידע
    loadData(1); // קוראים לפונקציית הטעינה לעמוד הראשון
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fetchData]); // התלות היא בפונקציית fetchData (שמגיעה מהאבא)
